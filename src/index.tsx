...
const skyboxes = [
  {
    label: "Abandoned Tank Farm",
    url: "./skybox/abandoned_tank_farm_01_2k.hdr",
  },
  ...
  { label: "Paul Lobe Haus", url: "./skybox/paul_lobe_haus_2k.hdr" },
  {
    ...
    url: "./skybox/reinforced_concrete_01_2k.hdr",
  },
...
const observerData: ObserverData = {
  ...
  skybox: {
    ...
    options: JSON.stringify(
      ["None"].concat(skyboxes.map((s) => s.label)).map((l) => ({ v: l, t: l }))
    ),
    exposure: 0,
    rotation: 0,
...
const main = () => {
  ...
  const absoluteUrl = (relative: string) =>
    new URL(relative, document.baseURI).toString()

  // create the graphics device
  createGraphicsDevice(canvas, {
    deviceTypes:
      url.searchParams.has("webgpu") || observer.get("enableWebGPU")
        ? ["webgpu"]
        : [],
